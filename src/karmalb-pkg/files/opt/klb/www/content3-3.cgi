...
if ( -e $filecluster )
{
	...
	if ( $action eq "Force node as backup for maintenance" )
	{
		...
		&logfile( "$ucarp $ignoreifstate -r $deadratio --interface=@rifname[0] --srcip=$lip --vhid=$idcluster --pass=secret --addr=$vipcl -k 100 --upscript=/opt/klb/app/zenlatency/zenlatency-start.pl --downscript=/opt/klb/app/zenlatency/zenlatency-stop.pl -B -f local6" );
		@eject = system ( "$ucarp $ignoreifstate -r $deadratio --interface=@rifname[0] --srcip=$lip --vhid=$idcluster --pass=secret --addr=$vipcl -k 100 --upscript=/opt/klb/app/zenlatency/zenlatency-start.pl --downscript=/opt/klb/app/zenlatency/zenlatency-stop.pl -B -f local6" );
		sleep ( 10 );
	}
	...
	if ( $action eq "Return node from maintenance" )
	{
		...
		if ( $typecl =~ /^equal$/ )
		{
			&logfile( "$ucarp $ignoreifstate -r $deadratio --interface=@rifname[0] --srcip=$lip --vhid=$idcluster --pass=secret --addr=$vipcl --upscript=/opt/klb/app/zenlatency/zenlatency-start.pl --downscript=/opt/klb/app/zenlatency/zenlatency-stop.pl -B -f local6" );
			my @eject = system ( "$ucarp $ignoreifstate -r $deadratio --interface=@rifname[0] --srcip=$lip --vhid=$idcluster --pass=secret --addr=$vipcl --upscript=/opt/klb/app/zenlatency/zenlatency-start.pl --downscript=/opt/klb/app/zenlatency/zenlatency-stop.pl -B -f local6" );
		}
		elsif ( $typecl =~ /$lhost-$rhost/ )
		{
			&logfile( "$ucarp $ignoreifstate -r $deadratio --interface=@rifname[0] --srcip=$lip -P --vhid=$idcluster --pass=secret --addr=$vipcl --upscript=/opt/klb/app/zenlatency/zenlatency-start.pl --downscript=/opt/klb/app/zenlatency/zenlatency-stop.pl -B -f local6" );
			my @eject = system ( "$ucarp $ignoreifstate -r $deadratio --interface=@rifname[0] --srcip=$lip -P --vhid=$idcluster --pass=secret --addr=$vipcl --upscript=/opt/klb/app/zenlatency/zenlatency-start.pl --downscript=/opt/klb/app/zenlatency/zenlatency-stop.pl -B -f local6" );
		}
		else
		{
			&logfile( "$ucarp $ignoreifstate -r $deadratio --interface=@rifname[0] -k 50 --srcip=$lip --vhid=$idcluster --pass=secret --addr=$vipcl --upscript=/opt/klb/app/zenlatency/zenlatency-start.pl --downscript=/opt/klb/app/zenlatency/zenlatency-stop.pl -B -f local6" );
			my $eject = system ( "$ucarp $ignoreifstate -r $deadratio --interface=@rifname[0] -k 50 --srcip=$lip --vhid=$idcluster --pass=secret --addr=$vipcl --upscript=/opt/klb/app/zenlatency/zenlatency-start.pl --downscript=/opt/klb/app/zenlatency/zenlatency-stop.pl -B -f local6" );
		}
		sleep ( 10 );
	}
	...
	if ( $action eq "Configure cluster type" && $typecl !~ /^$/ )
	{
		...
		if ( $? == 0 )
		{
			...
			if ( $typecl =~ /^equal$/ )
			{
				...
				sleep ( 10 );
				&logfile( "running on remote: $ucarp -r $deadratio $ignoreifstate --interface=@ifname[0] --srcip=$rip --vhid=$idcluster --pass=secret --addr=$vipcl --upscript=/opt/klb/app/zenlatency/zenlatency-start.pl --downscript=/opt/klb/app/zenlatency/zenlatency-stop.pl -B -f local6" );
